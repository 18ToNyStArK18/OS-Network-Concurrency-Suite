...
kfork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // Copy the actual tracking arrays.
...
kexit(int status)
{
  ...
  if(p->swap_file){
    // Log before starting the file system operation.
    printf("[pid %d] SWAPCLEANUP freed_slots=%d\n", p->pid, p->num_swappped_pages);
    
    begin_op(); // Start a file system transaction.
    
    // Truncate the file to free all its data blocks.
    itrunc(p->swap_file->ip);
    
    // Close the file handle. This will release the inode, and since
    // nlink will be 0, the file system will free the inode.
